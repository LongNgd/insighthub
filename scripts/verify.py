...
REPORT_NAME = re.compile(
    r"^(?:eval[-_]|cost[-_]|red-team[-_]|incident[-_]|audit\.|chatops-audit\.|day[1-7]\.json|source-manifest\.json)"
)
REQUIRED_TESTS = {
...
def real_text(value):
    ...
    return not re.search(
        r"(?i)^(?:TODO.*|placeholder|dummy|example|mock|fixture|fake|TBD|<[^>]+>|\.\.\.)$",
        value,
    )
...
def source_files(root):
    ...
    for folder in SOURCE_ROOTS:
        ...
        if not base.exists():
            continue
        need(
            not base.is_symlink() and base.resolve().is_relative_to(root.resolve()),
        ...
        for current, dirs, names in os.walk(base, followlinks=False):
            ...
            for name in names:
                ...
                if (
                    name.startswith(".env")
                    or name.endswith((".pyc", ".log", ".zip", ".html"))
                    or name == ".DS_Store"
                    or REPORT_NAME.match(name)
                ):
                    continue
...
def valid_url(value):
    ...
    need(
        ...
        "Expected HTTP(S) URL without credentials/fragment",
    )
    return value.rstrip("/")
...
def smoke(args, async_required=False):
    for url in (
        args.api_url + "/healthz",
        args.api_url + "/readyz",
        args.web_url + "/api/health",
    ):
    ...
    require(
        status in ({202} if async_required else {201, 202}),
        "Upload expected "
        + ("202 async" if async_required else "201 or 202")
        + ", got "
        + str(status),
    )
    if async_required:
        require(
            elapsed <= args.max_upload_seconds,
            "Async upload exceeded responsiveness budget",
        )
    doc = json_response(raw)
    require(
        isinstance(doc, dict)
        and type(doc.get("id")) in (str, int)
        and bool(str(doc["id"]))
        and not isinstance(doc["id"], bool),
        "Upload missing document id",
    )
    doc_id = doc["id"]
    if status == 201:
        ...
        require(
            doc.get("status") in {"pending", "queued", "processing", "ready"},
        ...
        while True:
            ...
            require(
                remaining > 0, "Document did not become ready within polling budget"
            )
            ...
            matches = [
                d
                for d in documents
                if isinstance(d, dict) and str(d.get("id")) == str(doc_id)
            ]
            ...
            if matches:
                ...
                require(
                    ...
                    "Polled document does not match this upload",
                )
                require(
                    doc.get("status") in {"pending", "queued", "processing", "ready"},
                    "Worker failed or returned unknown state",
                )
                if doc["status"] == "ready":
                    break
            time.sleep(min(args.poll_interval, max(0, deadline - time.monotonic())))
    require(
        type(doc.get("chunk_count")) is int and doc["chunk_count"] > 0,
        "Ready document has no chunks",
    )
    code, raw = http(
        args.api_url + "/chat",
        args.timeout,
        json.dumps({"question": args.question}).encode(),
    ...
    require(
        ...
        and isinstance(chat.get("answer"), str)
        and bool(chat["answer"].strip())
        and isinstance(chat.get("sources"), list)
        and bool(chat["sources"])
        and all(isinstance(x, str) and x.strip() for x in chat["sources"])
        and isinstance(chat.get("contexts"), list)
        and bool(chat["contexts"])
        and all(
            isinstance(x, dict)
            and any(
                isinstance(x.get(k), str) and x[k].strip()
                for k in ("chunk_text", "text", "content")
            )
            for x in chat["contexts"]
        ),
        "Chat needs nonempty answer, sources and contexts",
    )
    ...
    for line in metric_lines:
        match = re.fullmatch(
            r"insighthub_[a-zA-Z0-9_:]+(?:\{.*\})?\s+(\S+)(?:\s+\d+)?", line
        )
        if match:
            try:
...
def evidence(args, day):
    ...
    need(
        ...
        and type(data.get("day")) is int
        and data["day"] == day,
        "Wrong evidence schema/day",
    )
    need(
        data.get("mode") in {"real", "fixture"}, "Evidence mode must be real or fixture"
    )
    fresh(data.get("observed_at"), args.max_age_hours)
    need(
        data.get("source_sha256") == fingerprint(args.repo),
        "Evidence source digest differs from current content (including dirty edits)",
    )
    need(
        isinstance(data.get("artifacts"), dict), "Evidence artifacts must be an object"
    )
    return data
...
def artifact(args, data, role):
    ...
    if p.stat().st_size < 2048:
        try:
            ...
            need(
                real_text(content) and content not in {"{}", "[]"},
                "Known dummy artifact: " + role,
            )
        except UnicodeError:
            pass
    return p
...
def implemented_python(path):
    ...
    for function in functions:
        ...
        if body and any(
            ...
            and not (
                isinstance(n.func, ast.Name) and n.func.id == "NotImplementedError"
            )
...
def validate_junit(path, required):
    ...
    need(
        not any(c.find("skipped") is not None for c in cases),
        "Skipped/xfail tests do not complete a milestone",
    )
    names = {c.get("name", "").split("[")[0] for c in cases}
    need(
        required <= names,
        "Missing required scenarios: " + ", ".join(sorted(required - names)),
    )
    return len(cases)
...
def run_tests(args, day, scratch):
    ...
    env = dict(
        ...
        INSIGHTHUB_REPO_ROOT=str(args.repo),
        INSIGHTHUB_API_URL=args.api_url,
        INSIGHTHUB_WEB_URL=args.web_url,
        INSIGHTHUB_BOT_URL=args.bot_url,
        INSIGHTHUB_BOT_TRANSPORT=args.bot_transport,
        INSIGHTHUB_KUBE_CONTEXT=args.kube_context or "",
        INSIGHTHUB_NAMESPACE=args.namespace,
        INSIGHTHUB_VERIFY_RUN_ID=run_id,
        INSIGHTHUB_VERIFY_OBSERVATIONS=str(observations),
    )
    ...
    need(
        code not in {2, 3, 4, 5} and "No module named pytest" not in output,
...
def docker_args(args):
    return ["docker"] + (
        ["--context", args.docker_context] if args.docker_context else []
    )
...
def compose_args(args):
    result = docker_args(args) + [
        "compose",
        "-f",
        str(safe_path(args.repo, args.compose_file)),
    ]
...
def day1(args, data, scratch):
    ...
    need(
        real_text(prose.strip()) and len(prose.split()) >= 15,
        "Review artifact contains only headings/placeholders; explain change, risk and validation",
    )
    ...
    need(
        any(line.upper().startswith("FROM ") for line in instructions)
        and any(
            line.upper().startswith(("CMD ", "ENTRYPOINT ")) for line in instructions
        ),
        "Worker Dockerfile is a placeholder",
    )
    code, text = command(
        compose_args(args) + ["ps", "--format", "json", args.worker_service],
        scratch,
        args.timeout,
    )
    ...
    require(
        any(
            r.get("Service") == args.worker_service
            and r.get("State", "").lower() == "running"
            for r in records
        ),
        "Selected worker is not running",
    )
    ...
    require(
        matched,
        "No fresh ingestion_completed event for this upload in running worker logs",
    )
    return result
...
def day2(args, data, scratch):
    ...
    need(
        ...
        and all(real_text(t) for t in defaults + optional),
        "Manifest lacks supported tool capabilities",
    )
    supported = set(defaults + optional)
    requested = set(
        (args.mcp_tools if args.mcp_tools is not None else ",".join(defaults)).split(
            ","
        )
    )
    requested = {t.strip() for t in requested if t.strip()}
    need(
        requested and requested <= supported,
        "Requested MCP tools empty or unsupported by manifest",
    )
    ...
    for argv in (
        ["node", "--test", "--test-reporter=tap", str(core_test), str(server_test)],
        ["node", str(sdk)] + (["--live"] if data["mode"] == "real" else []),
    ):
        code, output = command(argv, scratch, args.test_timeout, env)
        need(
            code == 0 or not re.search(r"listen EACCES|listen EPERM", output),
            "Loopback socket permission unavailable for MCP integration tests",
        )
        need(
            not re.search(
                r"ERR_MODULE_NOT_FOUND|Cannot find package|Cannot find module", output
            ),
        ...
        if "--test" in argv:
            counts = {
                ...
                for key, value in re.findall(
                    r"^# (tests|pass|fail|skipped|todo) (\d+)$", output, re.M
                )
            }
            need(
                counts.get("tests", 0) > 0
                and counts.get("pass") == counts["tests"]
                and counts.get("fail") == 0
                and counts.get("skipped") == 0
                and counts.get("todo") == 0,
                "MCP tests missing, skipped or incomplete",
            )
    result = json_response(output.encode())
    need(
        isinstance(result, dict)
        and result.get("backend_mode")
        == ("live" if data["mode"] == "real" else "fixture")
        and result.get("live") is (data["mode"] == "real")
    ...
    need(
        isinstance(results, list) and results, "MCP smoke contains no SDK observations"
    )
    protocol = config.get("protocol")
    need(
        isinstance(protocol, dict)
        and real_text(protocol.get("modern"))
        and real_text(protocol.get("legacy")),
        "Manifest lacks supported protocol versions",
    )
    observed_modes = set()
    for item in results:
        need(
            isinstance(item, dict) and item.get("mode") not in observed_modes,
            "Duplicate/invalid MCP SDK observation",
        )
        mode = item.get("mode")
        need(
            mode in {"modern", "auto", "legacy", "v1"},
            "Unsupported SDK negotiation mode",
        )
        expected_protocol = (
            protocol["modern"] if mode in {"modern", "auto"} else protocol["legacy"]
        )
        require(
            item.get("protocol") == expected_protocol,
            "SDK negotiated protocol differs from manifest",
        )
        ...
        require(
            ...
            and {"tools/list", "tools/call"} <= set(methods),
        ...
        if data["mode"] == "real":
            require(
                set(calls) == requested,
                "Live MCP calls differ from requested capabilities",
            )
    return {
        "runtime_verified": data["mode"] == "real",
        "sdk_result": result,
...
def day3(args, data, scratch):
    ...
    expected = {
        "source_sha256": fingerprint(args.repo),
        "artifact_sha256": sha(deployment),
    }
    need(
        isinstance(binding, dict)
        and all(binding.get(k) == v for k, v in expected.items()),
        "CI binding does not match current source/artifact",
    )
    if args.ci_profile == "github":
        need(
            args.ci_repo
            and re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", args.ci_repo)
            and args.ci_run_id
            and args.ci_run_id.isdigit(),
            "Day 3 needs --ci-repo owner/repo and --ci-run-id NUMBER",
        )
    ...
    shutil.copytree(
        infra, target, ignore=shutil.ignore_patterns(".terraform", "*.tfstate*")
    )
    for cmd in (
    ...
    code, raw = command(
        [
            ...
            args.ci_repo,
            "--json",
            "status,conclusion,updatedAt,headSha,url",
        ],
        scratch,
        args.timeout,
    )
    ...
    require(
        live.get("status") == "completed" and live.get("conclusion") == "success",
        "Requested CI run is not successful",
    )
    ...
    require(
        ...
        "Live CI artifact belongs to different source/build",
    )
    return {
        "runtime_verified": True,
        "ci_url": live.get("url"),
...
def load_yaml(path):
    try:
        ...
        raise Incomplete(
            "PyYAML required for structured rule/test validation; install scripts/requirements-verification.txt"
        ) from exc
    try:
...
def validate_rule_tests(rules, tests):
    ...
    need(
        ...
        and isinstance(rule_data.get("groups"), list)
        and rule_data["groups"],
        "Rules contain no groups",
    )
    actual_rules = [
        r
        for g in rule_data["groups"]
        if isinstance(g, dict)
        for r in g.get("rules", [])
        if isinstance(r, dict)
        and real_text(r.get("expr"))
    ...
    need(
        ...
        and isinstance(test_data.get("tests"), list)
        and test_data["tests"],
        "Promtool test file contains no tests",
    )
    rule_files = test_data.get("rule_files")
    need(
        isinstance(rule_files, list)
        and rule_files
        and all(
            isinstance(f, str) and (tests.parent / f).resolve() == rules.resolve()
            for f in rule_files
        ),
    ...
    for case in test_data["tests"]:
        need(
            ...
            and isinstance(case.get("input_series"), list)
            and case["input_series"],
            "Rule test missing telemetry input series",
        )
        need(
            all(
                isinstance(series, dict)
                and real_text(series.get("series"))
                and real_text(series.get("values"))
                for series in case["input_series"]
            ),
            "Placeholder input series",
        )
        alerts, expressions = (
            case.get("alert_rule_test", []),
            case.get("promql_expr_test", []),
        )
        need(
            isinstance(alerts, list)
            and isinstance(expressions, list)
            and (alerts or expressions),
            "Rule test missing expected assertions",
        )
        for assertion in alerts:
            need(
                isinstance(assertion, dict)
                and real_text(assertion.get("alertname"))
                and "eval_time" in assertion
                and isinstance(assertion.get("exp_alerts"), list),
                "Invalid alert assertion",
            )
        for assertion in expressions:
            need(
                isinstance(assertion, dict)
                and real_text(assertion.get("expr"))
                and "eval_time" in assertion
                and isinstance(assertion.get("exp_samples"), list),
                "Invalid expression assertion",
            )
...
def verify_rca_component(args, data, scratch):
    ...
    need(
        ...
        and isinstance(dashboard.get("panels"), list)
        and any(
            isinstance(p, dict)
            and any(
                isinstance(t, dict) and real_text(t.get("expr"))
                for t in p.get("targets", [])
            )
            for p in dashboard["panels"]
        ),
        "Dashboard has no real query targets",
    )
    need(
        isinstance(rca, dict)
        and real_text(rca.get("incident_id"))
        and isinstance(rca.get("hypotheses"), list)
        and rca["hypotheses"]
        and all(real_text(x) for x in rca["hypotheses"]),
        "RCA needs incident and substantive hypotheses",
    )
    ...
    for sample in samples:
        need(
            ...
            and re.fullmatch(r"[a-zA-Z_:][a-zA-Z0-9_:]*", sample.get("metric", "")),
            "Invalid RCA metric name",
        )
        at = timestamp(sample.get("timestamp"))
        need(
            start <= at <= end and number(sample.get("value"), False),
            "Sample outside incident window or nonnumeric",
        )
        labels = sample.get("labels")
        need(
            isinstance(labels, dict)
            and all(
                re.fullmatch(r"[a-zA-Z_][a-zA-Z0-9_]*", k) and isinstance(v, str)
        ...
        selector = sample["metric"] + (
            "{"
            + ",".join(k + "=" + json.dumps(v) for k, v in sorted(labels.items()))
            + "}"
            if labels
            else ""
        )
        query = urllib.parse.urlencode(
            {"query": selector, "start": at, "end": at + 1, "step": 1}
        )
        code, raw = http(
            args.prometheus_url + "/api/v1/query_range?" + query, args.timeout
        )
        require(code == 200, "Prometheus query failed")
        response = json_response(raw)
        require(
            response.get("status") == "success"
            and response.get("data", {}).get("resultType") == "matrix",
            "Prometheus did not return range data",
        )
        matched = False
        for series in response["data"].get("result", []):
            for ts, value in series.get("values", []):
                if abs(float(ts) - at) < 1 and math.isclose(
                    float(value), sample["value"], rel_tol=1e-6, abs_tol=1e-9
                ):
                    matched = True
...
def day4(args, data, scratch):
    ...
    need(
        isinstance(dashboard, dict)
        and len(query_panels(dashboard.get("panels", []))) >= 9,
    ...
    need(
        all(isinstance(r, dict) and real_text(r.get("incident_id")) for r in reports)
        and len({r["incident_id"] for r in reports}) == 3,
        "Day 4 requires three distinct incident reports",
    )
    results = []
    for role in roles:
    ...
    return {
        "runtime_verified": all(r["runtime_verified"] for r in results),
        "incidents_checked": 3,
        "samples_checked": sum(r["samples_checked"] for r in results),
...
def audit_events(data, run_id=None, after=None):
    need(
        ...
        and data["events"],
        "Missing structured audit events",
    )
    decisions = set()
    for event in data["events"]:
        need(
            isinstance(event, dict)
            and all(real_text(event.get(k)) for k in ("event_id", "action", "user")),
            "Invalid audit event",
        )
        at = timestamp(event.get("timestamp"))
        need(at <= time.time() + 60, "Future audit event")
        need(
            event.get("decision") in {"allowed", "denied", "approval_required"},
            "Invalid audit decision",
        )
        if run_id:
            need(
                event.get("test_run_id") == run_id and at >= after - 1,
                "Audit not bound to this test run",
            )
        decisions.add(event["decision"])
    need(
        {"denied", "approval_required"} <= decisions,
        "Audit lacks permission/approval evidence",
    )
...
def day5(args, data, scratch):
    ...
    if args.bot_transport == "http":
        ...
        require(
            code == 200 and isinstance(json_response(raw), dict),
            "ChatOps health failed",
        )
    return {
        "runtime_verified": True,
        "tests": tests["count"],
...
def eval_report(report, dataset, digest, source, args, final):
    need(
        isinstance(report, dict) and report.get("mode") in {"real", "fixture"},
        "Evaluation missing mode",
    )
    fresh(report.get("observed_at"), args.max_age_hours)
    need(
        report.get("source_sha256") == source
        and report.get("dataset_sha256") == digest,
        "Evaluation source/dataset digest mismatch",
    )
    ...
    for result in results:
        need(
            isinstance(result, dict)
            and result.get("case_id") in dataset
            and result["case_id"] not in by_id,
            "Unknown/duplicate evaluation case",
        )
        need(
            type(result.get("passed")) is bool
            and result.get("severity") in {"low", "medium", "high", "critical"},
            "Evaluation needs boolean passed and severity",
        )
        for key in ("input_tokens", "output_tokens"):
            need(
                type(result.get(key)) is int and result[key] >= 0, "Invalid token count"
            )
        for key in ("provider", "model", "request_id"):
            need(
                (isinstance(result.get(key), str) and bool(result[key].strip()))
                if report["mode"] == "fixture"
                else real_text(result.get(key)),
                "Missing/placeholder evaluation provenance: " + key,
            )
        ...
        if report["mode"] == "real":
            need(
                not re.search(
                    r"(?i)(hash|extractive|fallback|fixture|mock|dummy|fake)",
                    result["provider"] + " " + result["model"],
                ),
                "Fallback is not real-model evaluation",
            )
        if final:
            require(
                result["passed"], "Final evaluation failed case: " + result["case_id"]
            )
...
def cost_report(cost, results, source, args):
    need(
        ...
        and cost.get("mode") in {"real", "fixture"}
        and cost.get("currency") == "USD",
        "Cost report missing mode/USD currency",
    )
    fresh(cost.get("observed_at"), args.max_age_hours)
    need(cost.get("source_sha256") == source, "Cost source digest mismatch")
    need(
        number(cost.get("budget_usd"))
        and cost["budget_usd"] > 0
        and number(cost.get("total_usd")),
        "Invalid budget/total",
    )
    ...
    for row in entries:
        need(
            isinstance(row, dict)
            and row.get("request_id") in expected
            and row["request_id"] not in seen,
            "Unknown/duplicate cost request",
        )
        result = expected[row["request_id"]]
        for key in ("input_tokens", "output_tokens"):
            need(
                type(row.get(key)) is int and row[key] == result[key],
                "Cost tokens differ from evaluation",
            )
        for key in ("input_usd_per_million", "output_usd_per_million", "cost_usd"):
            need(number(row.get(key)), "Invalid cost/rate")
        calculated = (
            row["input_tokens"] * row["input_usd_per_million"]
            + row["output_tokens"] * row["output_usd_per_million"]
        ) / 1_000_000
        require(
            math.isclose(calculated, row["cost_usd"], rel_tol=1e-6, abs_tol=1e-9),
            "Cost arithmetic mismatch",
        )
        if cost["mode"] == "real" and row["cost_usd"] == 0:
            usage = row.get("resource_usage")
            need(
                isinstance(usage, dict)
                and real_text(usage.get("measurement_source"))
                and number(usage.get("duration_seconds"))
                and usage["duration_seconds"] > 0
                and type(usage.get("memory_peak_bytes")) is int
                and usage["memory_peak_bytes"] > 0,
    ...
    require(
        math.isclose(total, cost["total_usd"], rel_tol=1e-6, abs_tol=1e-9),
        "Cost total mismatch",
    )
...
def day6(args, data, scratch):
    ...
    for row in dataset["cases"]:
        need(
            isinstance(row, dict)
            and all(
                real_text(row.get(k)) for k in ("id", "category", "input", "expected")
            ),
            "Invalid/placeholder dataset case",
        )
        need(row["id"] not in cases, "Duplicate dataset case id")
        cases[row["id"]] = row
    need(
        {"injection", "benign"} <= {row["category"] for row in cases.values()},
        "Dataset must test attacks and benign requests",
    )
    ...
    need(
        timestamp(initial["observed_at"]) <= timestamp(final["observed_at"]),
        "Final evaluation predates initial",
    )
    before = time.time()
    tests = run_tests(args, 6, scratch)
    actual = load_json(tests["observations"])
    need(
        isinstance(actual, dict) and actual.get("run_id") == tests["run_id"],
        "Evaluation observations not bound to this run",
    )
    actual_final, actual_cost = actual.get("eval_final"), actual.get("cost")
    live_results = eval_report(
        actual_final, cases, sha(dataset_file), source, args, True
    )
    cost_report(actual_cost, live_results, source, args)
    need(
        timestamp(actual_final["observed_at"]) >= before - 1
        and timestamp(actual_cost["observed_at"]) >= before - 1,
        "Tests reused stale evaluation/cost observations",
    )
    need(
        all(
            r["mode"] == data["mode"]
            for r in (initial, final, cost, actual_final, actual_cost)
        ),
        "Mixed real/fixture evidence modes",
    )
    return {
        "runtime_verified": data["mode"] == "real",
        "cases": len(cases),
...
def starter(args):
    ...
    need(
        isinstance(package, dict) and package.get("scripts"),
        "Web package scripts missing",
    )
    return {
        "runtime_verified": False,
        "note": "Starter file/syntax contract only. Run base tests and smoke separately; Days 1-6 remain student work.",
    }
...
def setup(args, scratch):
    ...
    code, output = command(
        ["node", "-p", 'process.versions.node.split(".")[0]'], scratch, args.timeout
    )
    need(
        code == 0 and output.strip().isdigit() and int(output.strip()) >= 20,
        "Node.js 20+ required",
    )
    code, _ = command(docker_args(args) + ["info"], scratch, args.timeout)
    need(code == 0, "Selected Docker daemon unavailable")
    code, output = command(
        compose_args(args) + ["config", "--format", "json"], scratch, args.timeout
    )
    require(code == 0, "Compose config failed")
    config = json_response(output.encode())
    require(
        {"api", "web", "postgres"} <= set(config.get("services", {})),
        "Compose missing core starter services",
    )
    return {
        "runtime_verified": False,
        "note": "Local tools and Compose configuration validated; application runtime not checked.",
    }
...
def parser():
    ...
    p.add_argument(
        ...
        nargs="?",
        default="starter",
        choices=["starter", "setup", "smoke", "fingerprint"]
    ...
    p.add_argument(
        "--api-url", default=os.environ.get("API_URL", "http://localhost:8000")
    )
    p.add_argument(
        "--web-url", default=os.environ.get("WEB_URL", "http://localhost:3000")
    )
    p.add_argument(
        "--bot-url", default=os.environ.get("BOT_URL", "http://localhost:8080")
    )
    ...
    p.add_argument(
        "--extended",
        action="store_true",
        help="Legacy alias; Day 1 always requires async worker",
    )
...
def main(argv=None):
    ...
    try:
        ...
        report.update(
            ...
            source_sha256=fingerprint(args.repo),
            scope="structure"
            if args.target in {"starter", "fingerprint"}
            else "environment"
            if args.target == "setup"
            else "partial-runtime-contract"
        ...
        if args.target == "fingerprint":
            ...
            with tempfile.TemporaryDirectory(
                prefix="insighthub-verify-", dir="/tmp"
            ) as tmp:
                for target in targets:
                    ...
                    try:
                        if target.startswith("day"):
                            ...
                            need(
                                not (day == 3 and args.ci_profile == "local"),
                                "Local preparation checked; GitHub pipeline remains mandatory for Day 3",
                            )
                            need(
                                data["mode"] == "real",
                                "Fixture checks completed; real milestone remains INCOMPLETE",
                            )
                        elif target == "smoke":
                            detail = smoke(args)
                        elif target == "setup":
                        ...
                        check.update(
                            status="INCOMPLETE",
                            message="Malformed artifact or unavailable dependency: "
                            + str(exc),
                        )
            ...
            report["runtime_verified"] = report["status"] == "PASS" and all(
                c["runtime_verified"] for c in report["checks"]
            )
            if args.target.startswith("day"):
                report["specification_review_required"] = True
                report["note"] = (
                    "PASS covers implemented checks only, not all project requirements; use the complete specification and reviewer evidence."
                )
            if args.target == "day7" and report["status"] == "PASS":
                report.update(status="INCOMPLETE", runtime_verified=False)
                report["checks"].append(
                    {
                        "id": "full-project-review",
                        "status": "INCOMPLETE",
        ...
        report["checks"].append(
            {"id": "input", "status": "INCOMPLETE", "message": str(exc)}
        )
    if "--json" in argv:
        ...
        print(
            f"{report['status']} {report['target']} (scope={report['scope']}, runtime_verified={str(report['runtime_verified']).lower()})"
        )
        for check in report["checks"]:
            print(
                f"  {check['status']} {check['id']}: {check.get('message', check.get('note', 'bounded contract satisfied'))}"
            )
