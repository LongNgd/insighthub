...
class VerifierCase(unittest.TestCase):
    def setUp(self):
        ...
        self.write(
            ...
            "A real sample document with some operational guidance.\n",
        )
    ...
    def envelope(self, day=1, mode="real", **changes):
        data = dict(
            ...
            mode=mode,
            observed_at=now(),
            source_sha256=v.fingerprint(self.root),
            artifacts={},
        )
        data.update(changes)
...
class CommonTests(VerifierCase):
    ...
    def test_missing_tool_and_timeout_are_incomplete(self):
        with (
            patch.object(v.shutil, "which", return_value=None),
            self.assertRaises(v.Incomplete),
        ):
    ...
    def test_junit_empty_failure_skipped_missing_case_rejected(self):
        ...
        for xml, error in [
            ("<testsuite/>", v.Incomplete),
            (
                '<testsuite><testcase name="test_permission_denied"><failure/></testcase></testsuite>',
                v.Failed,
            ),
            (
                '<testsuite><testcase name="test_permission_denied"><skipped/></testcase></testsuite>',
                v.Incomplete,
            ),
    ...
    def test_junit_valid_parametrized_cases(self):
        p = self.write(
            "result.xml",
            '<testsuite><testcase name="test_permission_denied[write]"/></testsuite>',
        )
        self.assertEqual(v.validate_junit(p, {"test_permission_denied"}), 1)

    def test_passed_text_cannot_override_test_exit(self):
        self.write(
            "tests/milestones/day1/test_worker.py",
            "def test_refactor_regression():\n    assert bool(1)\n",
        )
        with (
            patch.object(v, "command", return_value=(1, "100 passed; 1 failed")),
            self.assertRaises(v.Failed),
        ):
    ...
    def test_day1_without_worker_cannot_pass_by_default(self):
        ...
        self.ref(
            ...
            "review",
            "evidence/review.md",
            "This refactor changes validation with tests for empty and duplicate input. It does not implement the required asynchronous ingestion worker yet.",
        )
        with (
            patch.object(v, "run_tests"),
            patch.object(v, "smoke") as smoke,
            self.assertRaises(v.Incomplete),
        ):
    ...
    def test_day1_extended_missing_worker_is_incomplete(self):
        ...
        self.ref(
            ...
            "review",
            "evidence/review.md",
            "Refactor review covers changed validation, empty data handling, duplicate input and regression tests. Risk remains in queue behavior and needs additional runtime checks.",
        )
    ...
    def test_composite_preserves_failure_and_incomplete(self):
        ...
        with contextlib.ExitStack() as stack:
            for day in range(1, 7):
                stack.enter_context(
                    patch.object(
                        v, f"day{day}", return_value={"runtime_verified": True}
                    )
                )
            stack.enter_context(
                patch.object(v, "day3", side_effect=v.Failed("policy denied"))
            )
            stack.enter_context(
                patch.object(
                    v, "day5", side_effect=v.Incomplete("missing approval tests")
                )
            )
            code, report = self.cli("day7")
    ...
    def test_wrappers_portable_outside_repo(self):
        ...
        completed = subprocess.run(
            [
                "bash",
                str(script_dir / "verify-starter.sh"),
                "--repo",
                str(self.root),
                "--json",
            ],
...
class SmokeTests(VerifierCase):
    def responses(self, async_mode=False, **changes):
        ...
        def request(url, timeout, body=None, headers=None, method=None):
            ...
            if url.endswith("/documents") and body is not None:
                ...
                state["filename"] = (
                    re.search(rb'filename="([^"]+)"', body).group(1).decode()
                )
                return changes.get(
                    "upload_code", 202 if async_mode else 201
                ), json.dumps(
                    {
                        "id": 17,
            ...
            if url.endswith("/chat"):
                return 200, changes.get(
                    "chat",
                    b'{"answer":"Answer","sources":["doc"],"contexts":[{"text":"context"}]}',
                )
            if url.endswith("/metrics"):
                return 200, changes.get(
                    "metrics",
                    b"# HELP insighthub_requests desc\ninsighthub_requests 3\n",
                )
    ...
    def test_polling_is_bounded_and_does_not_accept_old_id(self):
        self.args.poll_timeout = 0.015
        self.args.poll_interval = 0.005
        for changes in ({"poll_state": "pending"}, {"poll_id": 16}):
            request, _ = self.responses(True, **changes)
            start = time.monotonic()
            with (
                patch.object(v, "http", side_effect=request),
                self.assertRaises(v.Failed),
            ):
                v.smoke(self.args)
    ...
    def test_malformed_empty_chat_and_comment_metrics_fail(self):
        for changes in (
            {"chat": b"not json"},
            {"chat": b'{"answer":"","sources":[],"contexts":[]}'},
    ...
    def test_unavailable_endpoint_is_incomplete(self):
        with patch.object(
            v.urllib.request.OpenerDirector,
            "open",
            side_effect=v.urllib.error.URLError("down"),
        ):
            with self.assertRaises(v.Incomplete):
                v.http("http://localhost:1/healthz", 0.1)
...
class DomainTests(VerifierCase):
        ...
        results = [
            {
                "case_id": key,
                "passed": True,
                "severity": "high" if key == "attack" else "low",
                "provider": provider,
                "model": "qwen3:8b",
        ...
        cost = dict(
            ...
            entries=[
                dict(
                    ...
                    cost_usd=0,
                    resource_usage=dict(
                        measurement_source="docker stats captured for evaluation",
    ...
    def test_fixture_report_can_validate_but_is_labelled(self):
        ...
        self.assertEqual(
            len(v.eval_report(report, cases, "b" * 64, "a" * 64, self.args, True)), 2
        )
    ...
    def test_cost_mismatch_nan_duplicate_and_budget(self):
        for mutation, error in [
            ...
            if mutation == "total":
                ...
                cost["entries"][0]["cost_usd"] = float("nan")
            elif mutation == "duplicate":
                cost["entries"].append(cost["entries"][0])
            else:
                cost["entries"][0]["input_tokens"] = 99
    ...
    def test_local_ci_does_not_call_github(self):
        ...
        deployment = self.ref(
            data, "deployment", "evidence/deployment.bin", "actual build bytes 01891"
        )
        self.write("infra/main.tf", "terraform {}\n")
        self.ref(
            data,
            "ci_binding",
            "evidence/ci_binding.json",
            {
                "source_sha256": v.fingerprint(self.root),
                "artifact_sha256": v.sha(deployment),
            },
        )
        calls = []
    ...
    def test_ci_policy_exit_cannot_be_hidden_by_passed_text(self):
        ...
        data = self.envelope(3)
        deployment = self.ref(
            data, "deployment", "evidence/deployment.bin", "build bytes"
        )
        self.write("infra/main.tf", "terraform {}\n")
        self.ref(
            data,
        ...
        with (
            patch.object(v, "run_tests"),
            patch.object(v, "command", return_value=(1, "Passed checks: 100")),
            self.assertRaises(v.Failed),
        ):
...
class McpContractTests(VerifierCase):
    def mcp(self, mode="real"):
        ...
        result = dict(
            passed=True,
            backend_mode="live" if mode == "real" else "fixture",
            live=mode == "real",
            backend="live-loopback" if mode == "real" else "fixture-loopback",
            results=[
                dict(
                    mode="modern",
                    protocol="2026-07-28",
                    methods=["server/discover", "tools/list", "tools/call"],
                    calls=["insighthub_health"],
                    passed=True,
                )
            ],
        )
    ...
    def test_no_calls_or_protocol_mismatch_fails(self):
        for mutation in ("empty", "protocol"):
            data, result = self.mcp()
            if mutation == "empty":
                result["results"][0]["calls"] = []
            else:
                result["results"][0]["protocol"] = "unexpected"
            with (
    ...
    def test_sdk_missing_dependency_and_skips_are_incomplete(self):
        for code, text in [
            ...
            with (
                ...
                patch.object(v, "command", return_value=(code, text)),
                self.assertRaises(v.Incomplete),
            ):
                v.day2(self.args, data, self.scratch)
...
class TelemetryTests(VerifierCase):
    def telemetry(self):
        ...
        self.ref(
            ...
            {"panels": [{"targets": [{"expr": "insighthub_errors_total"}]}]},
        )
        at = now(-10)
        self.ref(
            data,
            ...
            "evidence/incident.json",
            {
                ...
                "ended_at": now(-5),
                "hypotheses": [
                    "Queue backlog preceded the latency increase; inspect retry amplification."
                ],
                "samples": [
                    {
                        "metric": "insighthub_errors_total",
                        "labels": {"job": "api"},
                        "timestamp": at,
                        "value": 3,
                    }
        ...
        response = {
            ...
            "data": {
                ...
                "result": [
                    {
                        "metric": {"__name__": "insighthub_errors_total", "job": "api"},
    ...
    def test_empty_rule_groups_or_tests_not_success(self):
        ...
        with (
            patch.object(v, "load_yaml", side_effect=[{"groups": []}, {"tests": []}]),
            self.assertRaises(v.Incomplete),
        ):
        ...
        with (
            patch.object(
                ...
                side_effect=[
                    {
                        "groups": [
                            {"rules": [{"alert": "HighErrors", "expr": "errors > 0"}]}
                        ]
                    },
                    {
                        "rule_files": ["rules.yml"],
                        "tests": [
                            {"input_series": [{"series": "errors", "values": "0 1 2"}]}
                        ],
                    },
                ],
            ),
    ...
    def test_live_timestamped_telemetry_matches(self):
        ...
        with (
            patch.object(v, "validate_rule_tests"),
            patch.object(v, "command", return_value=(0, "")),
            patch.object(v, "http", return_value=(200, json.dumps(response).encode())),
        ):
            self.assertTrue(
    ...
    def test_stale_or_mismatched_telemetry_not_accepted(self):
        for change in ("timestamp", "value", "empty"):
            data, response = self.telemetry()
            if change == "timestamp":
                response["data"]["result"][0]["values"][0][0] -= 60
            elif change == "value":
                response["data"]["result"][0]["values"][0][1] = "99"
            else:
            ...
            with (
                ...
                patch.object(v, "command", return_value=(0, "")),
                patch.object(
                    v, "http", return_value=(200, json.dumps(response).encode())
                ),
                self.assertRaises(v.Failed),
            ):
                v.verify_rca_component(self.args, data, self.scratch)
    ...
    def test_promtool_nonzero_not_overridden_by_success_text(self):
        ...
        with (
            patch.object(v, "validate_rule_tests"),
            patch.object(v, "command", return_value=(1, "SUCCESS")),
            self.assertRaises(v.Failed),
        ):
            v.verify_rca_component(self.args, data, self.scratch)
...
class ExtraGuards(VerifierCase):
    ...
    def test_detailed_junit_not_pass_string_is_required(self):
        self.write(
            "tests/milestones/day1/test_core.py",
            "def test_core():\n    assert bool(1)\n",
        )
        with (
            patch.object(v, "command", return_value=(0, "3 passed")),
            self.assertRaises(v.Incomplete),
        ):
            v.run_tests(self.args, 1, self.scratch)
...
class RestoredProjectScopeTests(VerifierCase):
    ...
    def test_one_rca_or_one_panel_cannot_pass_day4(self):
        ...
        self.ref(
            ...
            "dashboard",
            "evidence/dashboard.json",
            {"panels": [{"targets": [{"expr": "up"}]} for _ in range(9)]},
        )
        self.ref(
            data,
            "mlops_notes",
            "evidence/mlops.md",
            "Registry, approval gate, drift and rollback with ownership explained.",
        )
        self.ref(
            data, "rca", "evidence/incident1.json", {"incident_id": "latency-spike"}
        )
        with self.assertRaises(v.Incomplete):
            v.day4(self.args, data, self.scratch)
    ...
    def test_all_three_rca_are_checked(self):
        ...
        for n, role in enumerate(("rca", "rca_2", "rca_3")):
            self.ref(
                data,
                role,
                f"evidence/incident{n}.json",
                {"incident_id": f"incident-{n}"},
            )
        with patch.object(
            v,
            "verify_rca_component",
            return_value={"runtime_verified": True, "samples_checked": 1},
        ) as probe:
            self.assertEqual(
                v.day4(self.args, data, self.scratch)["incidents_checked"], 3
            )
    ...
    def test_automated_subset_cannot_complete_project(self):
        ...
        with contextlib.ExitStack() as stack:
            for n in range(1, 7):
                stack.enter_context(
                    patch.object(v, f"day{n}", return_value={"runtime_verified": True})
                )
            code, report = self.cli("day7")
        self.assertEqual(code, 2)
